(function (TDI) {
		...
		TDI.Ajax.send($('#tdi-ajax-send-href'), {
			beforeStart: function (xhr, settings, options) {
				...
				assert.equal(
					...
					url.replace(/&_ts=\d+/, ''),
					'The AJAX url is the same as the `href` attribute.'
				);
		...
		TDI.Ajax.send($('#tdi-ajax-send-ajax-url'), {
			beforeStart: function (xhr, settings, options) {
				...
				assert.equal(
					settings.url.replace(/&_ts=\d+/, ''),
					url.replace(/&_ts=\d+/, ''),
					'The AJAX url is the same as the `data-ajax-url` attribute.'
				);
	...
	QUnit.test('TDI.Ajax.send: Form/Action', function (assert) {
		...
		TDI.Ajax.send($('#tdi-ajax-send-form-action'), {
			beforeStart: function (xhr, textStatus, options) {
				var url = $('#tdi-ajax-send-form-action').attr('action');

				assert.equal(
					options.url.replace(/&_ts=\d+/, ''),
					TDI.Ajax.Request.ajaxifyUrl(url).replace(/&_ts=\d+/, ''),
					'The AJAX url is the same as the `action` attribute.'
				);
	...
	QUnit.test('TDI.Ajax.send: Form/Data-ajax-url', function (assert) {
		...
		TDI.Ajax.send($('#tdi-ajax-send-form-ajax-url'), {
			beforeStart: function (xhr, textStatus, options) {
				var url = $('#tdi-ajax-send-form-ajax-url').data('ajax-url');
	...
	QUnit.test('TDI.Ajax.send: setting/unsetting of the class names', function (assert) {
		...
		var $relatedAncestor = $('#related-ancestor');
		var $relatedElement = $('#related-element');
		var $rel = $('#deprecated-related-element');

		TDI.Ajax.send($lnk, {
			start: function () {
				assert.ok(
					$lnk.hasClass('loading'),
					'The element has the `loading` class name.'
				);
				assert.ok(
					$relatedAncestor.hasClass('loading-target'),
					'The related ancestor element has the `loading-target` class name.'
				);
				assert.ok(
					$relatedElement.hasClass('loading-target'),
					'The related element has the `loading-target` class name.'
				);
				assert.ok(
					$rel.hasClass('loading-target'),
					'The deprecated related ancestor element has the `loading-target` class name.'
				);
			},
				...
				assert.ok(
					!$lnk.hasClass('loading'),
					'The element does not have the `loading` class name.'
				);
				assert.ok(
					!$relatedAncestor.hasClass('loading-target'),
					'The related ancestor element does not have the `loading` class name.'
				);
				assert.ok(
					!$relatedElement.hasClass('loading-target'),
					'The related element does not have the `loading` class name.'
				);
				assert.ok(
					!$rel.hasClass('loading-target'),
					'The deprecated related ancestor element does not have the `loading` class name.'
				);
