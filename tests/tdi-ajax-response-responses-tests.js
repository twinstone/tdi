	...
	QUnit.test('TDI.Ajax.Response: events and XML', function (assert) {
		...
		$(document)
			...
			.bind('tdi:ajax:updatesDone', function (evt, data) {
				...
				var u2 = data.updates[1];
				var u3 = data.updates[2];
				var u4 = data.updates[3];
				var u5 = data.updates[4];
				var u6 = data.updates[5];
				var u7 = data.updates[6];
				var u8 = data.updates[7];
				var u9 = data.updates[8];
				var u10 = data.updates[9];
				...
				assert.equal(
					u7.selector,
					'#update-multiple-1, #update-multiple-2, .update-multiple',
					'Update7: selector'
				);
				...
				assert.equal(
					$('#update-default').text(),
					'Update default',
					'The target was updated and has the correct contents.'
				);
				assert.equal(
					$('#update-replace').length,
					0,
					'The target does not exist, it was replaced.'
				);
				assert.equal(
					$('#update-replaced').length,
					1,
					'A new element `update-replace` replaced the former update target.'
				);
				assert.equal(
					$('#update-replaced').text(),
					'Update replace',
					'The new element has the correct contents.'
				);
				assert.equal(
					$('#update-append').text(),
					'This is the first text.Update append',
					'The target was updated and new contents were appended at the end.'
				);
				assert.equal(
					$('#update-prepend').text(),
					'Update prependThis is the last text.',
					'The target was updated and new contents were prepended at the beginning.'
				);
				assert.ok(
					$('#update-add-class').hasClass('cls-add'),
					'A new class name was added to the target.'
				);
				assert.equal(
					$('#update-add-class').text(),
					'Update add class'
				);
				assert.ok(
					!$('#update-remove-class').hasClass('cls-remove'),
					'A class name was removed from the target.'
				);
				assert.equal(
					$('#update-multiple-1').text(),
					'Update multiple',
					'The multiple target 1 was updated and has the correct contents.'
				);
				assert.equal(
					$('#update-multiple-2').text(),
					'Update multiple',
					'The multiple target 2 was updated and has the correct contents.'
				);
				assert.equal(
					$('.update-multiple').eq(0).text(),
					'Update multiple',
					'The multiple target 3 was updated and has the correct contents.'
				);
				assert.equal(
					$('.update-multiple').eq(1).text(),
					'Update multiple',
					'The multiple target 4 was updated and has the correct contents.'
				);
				assert.ok(
					$('#update-multiple-1').hasClass('cls-add'),
					'A new class name was added to the multiple target 1'
				);
				assert.ok(
					$('#update-multiple-2').hasClass('cls-add'),
					'A new class name was added to the multiple target 2'
				);
				assert.ok(
					$('.update-multiple').eq(0).hasClass('cls-add'),
					'A new class name was added to the multiple target 3'
				);
				assert.ok(
					$('.update-multiple').eq(1).hasClass('cls-add'),
					'A new class name was added to the multiple target 4'
				);
				assert.ok(
					!$('#update-multiple-1').hasClass('cls-remove'),
					'A class name was removed from the multiple target 1'
				);
				assert.ok(
					!$('#update-multiple-2').hasClass('cls-remove'),
					'A class name was removed from the multiple target 2'
				);
				assert.ok(
					!$('.update-multiple').eq(0).hasClass('cls-remove'),
					'A class name was removed from the multiple target 3'
				);
				assert.ok(
					!$('.update-multiple').eq(1).hasClass('cls-remove'),
					'A class name was removed from the multiple target 4'
				);
				assert.equal(
					$('.update-multiple-replace').length,
					0,
					'The targets do not exist, they were replaced.'
				);
				assert.equal(
					$('.update-multiple-replaced').length,
					2,
					'New elements `update-multiple-replaced` replaced the former update targets.'
				);
				assert.equal(
					$('.update-multiple-replaced').eq(0).text(),
					'Update multiple replace',
					'The new element has the correct contents.'
				);
				assert.equal(
					$('.update-multiple-replaced').eq(1).text(),
					'Update multiple replace',
					'The new element has the correct contents.'
				);
				assert.equal(
					$('#update-multiple-7').text(),
					'This is the first text.Update append',
					'The multiple target was updated and new contents were appended at the end.'
				);
				assert.equal(
					$('#update-multiple-8').text(),
					'This is the first text.Update append',
					'The multiple target was updated and new contents were appended at the end.'
				);
				assert.equal(
					$('#update-multiple-9').text(),
					'Update prependThis is the last text.',
					'The multiple target was updated and new contents were prepended at the beginning.'
				);
				assert.equal(
					$('#update-multiple-10').text(),
					'Update prependThis is the last text.',
					'The multiple target was updated and new contents were prepended at the beginning.'
				);
			})
			.bind('tdi:ajax:insertsDone', function (evt, data) {
				...
				var i2 = data.inserts[1];
				var i3 = data.inserts[2];
				var i4 = data.inserts[3];
				var i5 = data.inserts[4];
				var i6 = data.inserts[5];
				...
				assert.equal(
					i1.content,
					'<div id="inserted-after-default">Inserted after (default)</div>',
					'Insert1: content'
				);
				assert.equal(
					i1.position,
				...
				assert.equal(
					i1.inserted_node.attr('id'),
					'inserted-after-default',
					'Insert1: inserted node'
				);
				...
				assert.equal(
					i2.content,
					'<div id="inserted-after">Inserted after</div>',
					'Insert2: content'
				);
				assert.equal(
					i2.position,
				...
				assert.equal(
					i2.inserted_node.attr('id'),
					'inserted-after',
					'Insert2: inserted node'
				);
				...
				assert.equal(
					i3.content,
					'<div id="inserted-before">Inserted before</div>',
					'Insert3: content'
				);
				assert.equal(
					i3.position,
				...
				assert.equal(
					i3.inserted_node.attr('id'),
					'inserted-before',
					'Insert3: inserted node'
				);

				assert.equal(
					i4.selector,
					'.inserts-multiple-default',
					'Insert4: selector'
				);
				assert.equal(
					i4.target.length,
				...
				assert.equal(
					i4.content,
					'<div class="inserted-multiple-after-default">Inserted after (default)</div>',
					'Insert4: content'
				);
				assert.equal(
					i4.position,
				...
				assert.equal(
					i4.inserted_node.length,
					2,
					'Insert4: inserted nodes'
				);

				assert.equal(
					i5.selector,
					'.inserts-multiple-after',
					'Insert5: selector'
				);
				assert.equal(
					i5.target.length,
				...
				assert.equal(
					i5.content,
					'<div class="inserted-multiple-after">Inserted after</div>',
					'Insert5: content'
				);
				assert.equal(
					i5.position,
				...
				assert.equal(
					i5.inserted_node.length,
					2,
					'Insert5: inserted nodes'
				);

				assert.equal(
					i6.selector,
					'.inserts-multiple-before',
					'Insert6: selector'
				);
				assert.equal(
					i6.target.length,
				...
				assert.equal(
					i6.content,
					'<div class="inserted-multiple-before">Inserted before</div>',
					'Insert6: content'
				);
				assert.equal(
					i6.position,
				...
				assert.equal(
					i6.inserted_node.length,
					2,
					'Insert6: inserted nodes'
				);
				...
				assert.equal(
					$('#inserts').next('#inserted-after').next('#inserted-after-default').length,
					1,
					'Inserted after: correct position'
				);
				assert.equal(
					$('#inserts').prev('#inserted-before').length,
					1,
					'Inserted before: correct position'
				);
				assert.equal(
					$('.inserted-multiple-after-default').length,
					2,
					'Inserted multiple after (default)'
				);
				assert.equal(
					$('#inserts-multiple-default-1').next('.inserted-multiple-after-default').length,
					1,
					'Inserted multiple after (default): correct position'
				);
				assert.equal(
					$('#inserts-multiple-default-2').next('.inserted-multiple-after-default').length,
					1,
					'Inserted multiple after (default): correct position'
				);
				assert.equal(
					$('.inserted-multiple-after').length,
					2,
					'Inserted multiple after'
				);
				assert.equal(
					$('#inserts-multiple-after-1').next('.inserted-multiple-after').length,
					1,
					'Inserted multiple after: correct position'
				);
				assert.equal(
					$('#inserts-multiple-after-2').next('.inserted-multiple-after').length,
					1,
					'Inserted multiple after: correct position'
				);
				assert.equal(
					$('.inserted-multiple-before').length,
					2,
					'Inserted multiple before'
				);
				assert.equal(
					$('#inserts-multiple-before-1').prev('.inserted-multiple-before').length,
					1,
					'Inserted multiple before: correct position'
				);
				assert.equal(
					$('#inserts-multiple-before-2').prev('.inserted-multiple-before').length,
					1,
					'Inserted multiple before: correct position'
				);
			})
...
\n\
			...
			.bind('tdi:ajax:popupsDone', function (evt, data) {
				...
				var p2 = data.popups[1];
				var p3 = data.popups[2];
			...
			.bind('tdi:ajax:unknownsDone', function (evt, data) {
				...
				var u2 = data.unknowns[1];
				var u3 = data.unknowns[2];
				var u4 = data.unknowns[3];
				var u5 = data.unknowns[4];
