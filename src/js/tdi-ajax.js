...
TDI.Ajax = function($) {
	...
	function _bindUI() {
		$(document)
			...
			.delegate( _delegateSelectors.linkClick, 'click', _onLinkClick )
				...
				var iframeName = 'form_iframe_'+(new Date()).getTime(),
					...
					// onComplete/onEnd
						$(iframe).load( function() {
							...
							$(document).trigger( 'tdi:ajax:_success', { data : xml, textStatus : '', xhr : null, options : options } );
...
TDI.Ajax.Response = function($) {
				...
				$xml.find( infusionInstructionsNames.join( ',' ) ).each( function() {
					...
					switch( instruction ) {
						case 'Script':
							...
							break;
						default:
								_infusionInstructions[ instruction ]( this );
							break;
					}
				} );
				
				// download and execute the list of script tags
					_onBeforeScript( _scriptTags.shift() );
	...
	// RESPONSES -----------------------------------------------------------------
		 ...
		 */
		function _onBeforeRedirect( tag ) {
			if ( !tag ) { return false; }
